 ...
 *
 * 支持的指令：
 * - hash: 在upstream块内启用哈希负载均衡
 *   语法: hash key [consistent];
 *   上下文: upstream
 *
 * 支持的变量：
 * $upstream_hash_key: 用于生成哈希值的键
 *
 * 使用注意点：
 * 1. 确保在upstream块内正确配置hash指令
 * 2. 选择合适的哈希键，以实现期望的负载分配
 * 3. 使用consistent参数可以启用一致性哈希，提高节点变化时的稳定性
 * 4. 合理设置服务器权重，以影响哈希分布
 * 5. 注意哈希算法的性能影响，特别是在大规模部署时
 * 6. 考虑结合其他模块（如健康检查）使用，以提高负载均衡的可靠性
 * 7. 在使用自定义哈希键时，确保键的唯一性和分布性
 * 8. 定期监控负载均衡效果，根据实际情况调整配置
 */
