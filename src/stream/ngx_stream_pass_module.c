 ...
 *
 * 支持的变量：
 * 该模块不定义特定变量，但可以在pass指令中使用复杂值表达式
 *
 * 使用注意点：
 * 1. 确保传递的目标地址是有效的监听端口
 * 2. 避免创建传递循环，模块有最大传递次数限制(NGX_STREAM_PASS_MAX_PASSES)
 * 3. 传递操作会创建新的连接，注意资源使用
 * 4. 在使用动态地址时，确保解析结果是有效的
 * 5. 传递后原始会话将被关闭，后续处理将在新会话中进行
 * 6. 注意传递操作可能影响日志记录和统计
 * 7. 在SSL/TLS连接中使用时，传递会发生在握手之前
 * 8. 合理使用可提高负载均衡灵活性，但过度使用可能增加复杂性
 * 9. 在高并发场景下，注意传递操作对性能的影响
 * 10. 定期检查日志，确保传递行为符合预期
 */
