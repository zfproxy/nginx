 ...
 *
 * 支持的指令：
 * - least_conn: 在upstream块内启用最少连接负载均衡
 *   语法: least_conn;
 *   上下文: upstream
 *
 * 支持的变量：
 ...
 *
 * 使用注意点：
 * 1. 确保在upstream块内正确配置least_conn指令
 * 2. 最少连接算法适用于处理时间较长的连接场景
 * 3. 与其他负载均衡算法（如round-robin）相比，可能会增加一些额外的计算开销
 * 4. 在高并发场景下，需要注意负载均衡的效果和性能影响
 * 5. 考虑结合健康检查机制使用，以避免将请求发送到不健康的服务器
 * 6. 如果上游服务器的处理能力差异较大，可能需要结合权重配置使用
 * 7. 定期监控负载均衡效果，根据实际情况调整配置
 */
