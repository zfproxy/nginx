 ...
 * 支持的指令：
 * - map: 定义一个映射
 *   语法: map $variable $new_variable { ... }
 *   上下文: stream
 * - map_hash_max_size: 设置映射哈希表的最大大小
 *   语法: map_hash_max_size size;
 *   默认值: map_hash_max_size 2048;
 *   上下文: stream
 * - map_hash_bucket_size: 设置映射哈希桶的大小
 *   语法: map_hash_bucket_size size;
 *   默认值: map_hash_bucket_size 32|64|128;
 *   上下文: stream
 *
 * 支持的变量：
 * 该模块不定义特定变量，但可以创建新的变量
 *
 * 使用注意点：
 * 1. 合理设置哈希表大小，避免内存浪费或性能下降
 * 2. 复杂的映射规则可能影响性能，建议使用缓存
 * 3. 正则表达式匹配比精确匹配更耗资源，谨慎使用
 * 4. 默认值应当覆盖所有可能的情况
 * 5. 主机名匹配时注意通配符的使用
 * 6. 映射结果可能被缓存，动态改变源数据可能不会立即生效
 * 7. 大量使用map可能增加配置复杂度，影响可维护性
 * 8. 注意变量名的命名，避免冲突
 * 9. 在高并发场景下，注意map操作对性能的影响
 * 10. 定期检查和优化映射规则，删除不再使用的映射
 */
