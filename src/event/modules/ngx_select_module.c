 ...
 *
 * 使用注意点:
 * 1. select方法在大量文件描述符时性能会显著下降，不适合高并发场景
 * 2. 对于支持更高效I/O多路复用机制(如epoll、kqueue)的系统，建议使用那些机制
 * 3. select有文件描述符数量的限制，通常为1024（可通过修改系统限制提高）
 * 4. 每次调用select都需要重新设置文件描述符集，可能导致性能开销
 * 5. select是水平触发的，可能导致某些情况下的惊群效应
 * 6. 在使用select时，需要注意处理EINTR错误（被信号中断）
 * 7. 对于长连接场景，select的性能表现可能不如其他事件驱动机制
 * 8. 在配置Nginx时，如果系统支持其他更高效的事件模块，应优先选择那些模块
 */
