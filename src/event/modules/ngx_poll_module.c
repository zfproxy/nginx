 ...
 *
 * 使用注意点:
 * 1. poll方法在大量文件描述符时性能可能下降，不适合高并发场景
 * 2. 对于支持更高效I/O多路复用机制(如epoll、kqueue)的系统，建议使用那些机制
 * 3. 每次调用poll都需要传递完整的文件描述符列表，可能导致内核态和用户态数据拷贝开销较大
 * 4. poll没有就绪队列，每次唤醒都需要遍历所有文件描述符，效率较低
 * 5. 在使用poll时，需要注意系统对文件描述符数量的限制
 * 6. 对于长连接场景，poll的性能表现可能不如其他事件驱动机制
 * 7. 在配置Nginx时，如果系统支持其他更高效的事件模块，应优先选择那些模块
 * 8. 定期检查和更新Nginx，以获取poll模块的最新优化和bug修复
 */
