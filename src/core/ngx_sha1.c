...
/**
 * @file ngx_sha1.c
 * @brief Nginx SHA1哈希算法实现
 *
 * 本文件实现了Nginx的SHA1哈希算法功能。
 *
 * 主要功能:
 * - SHA1哈希计算初始化
 * - 数据块的SHA1哈希更新
 ...
 * 使用注意:
 * 1. 确保正确初始化SHA1上下文结构体
 * 2. 大数据量时应分块调用ngx_sha1_update函数
 * 3. 所有数据处理完毕后调用ngx_sha1_final获取最终哈希值
 * 4. 注意字节序问题,特别是在跨平台应用时
 * 5. SHA1算法已不再被认为是密码学安全的,不应用于安全敏感场景
 * 6. 处理敏感数据时,注意清理中间状态和临时缓冲区
 * 7. 在多线程环境中使用时,需确保每个线程使用独立的上下文结构体
 */
