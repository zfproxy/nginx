 ...
 *
 * 本文件实现了基于Unix系统的UDP数据包发送功能。
 *
 * 主要功能:
 * 1. 使用sendmsg()系统调用发送UDP数据包
 * 2. 支持链式缓冲区结构的数据发送
 * 3. 实现了向量化I/O (iovec)以提高发送效率
 * 4. 处理部分发送的情况
 * 5. 支持设置发送数据的大小限制
 *
 * 使用注意事项:
 * 1. 确保在调用发送函数前已正确初始化UDP连接
 * 2. 注意处理返回的链表，它可能包含未发送完的数据
 * 3. 发送大小限制(limit参数)应根据实际需求合理设置
 * 4. 在非阻塞模式下，需要正确处理EAGAIN错误
 * 5. 注意检查发送后的错误状态，特别是在kqueue事件模型下
 */
