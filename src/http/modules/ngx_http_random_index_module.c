 ...
 *
 * 使用注意点:
 * 1. 只有在没有找到index指令指定的文件时，random_index才会生效
 * 2. 确保目录中有可访问的文件，否则可能会返回403错误
 * 3. 随机选择可能会影响性能，特别是在包含大量文件的目录中
 * 4. 考虑使用try_files指令作为备选方案
 * 5. 注意文件系统权限，确保Nginx有权限读取目录和文件
 * 6. 在生产环境中谨慎使用，可能会暴露不应公开的文件
 * 7. 与autoindex指令一起使用时要小心，可能会产生意外结果
 * 8. 定期检查日志，确保随机索引功能按预期工作
 */
