 ...
 *   参数: 
 *     two: 启用两次随机选择
 *     method: 两次选择的方法，可以是 least_conn 或 least_time
 *
 * 提供的变量:
 * 该模块不提供额外的变量
 *
 * 使用注意点:
 * 1. 随机负载均衡适用于上游服务器性能相近的情况
 * 2. 使用 "two" 参数可以在一定程度上避免选中性能较差的服务器
 * 3. 当使用 "two" 参数时，会稍微增加选择服务器的开销
 * 4. 权重设置会影响服务器被选中的概率
 * 5. 与其他负载均衡方法相比，随机方法可能导致负载分布不够均匀
 * 6. 在高并发场景下，随机方法可能会导致某些服务器短时间内接收过多请求
 */
