 ...
 * - $connections_writing: 正在写响应的连接数
 * - $connections_waiting: 等待请求的空闲连接数
 *
 * 使用注意点:
 * 1. 为了安全考虑，建议限制对状态页面的访问，例如使用allow和deny指令
 * 2. 状态页面的频繁访问可能会对性能产生轻微影响
 * 3. 在高并发环境下，显示的数据可能会有轻微延迟
 * 4. 该模块不提供历史数据或详细的性能分析，仅显示当前状态
 * 5. 在使用反向代理时，要注意区分前端和后端服务器的状态
 * 6. 可以结合其他监控工具使用，以获得更全面的服务器状态信息
 * 7. 状态页面的输出格式是纯文本，便于其他程序解析和处理
 */
